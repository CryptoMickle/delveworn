...
The frontend source now serves a neutral mode-selection home at `/`, the current wallet-free challenge at `/challenge`, local Practice at `/practice`, the experimental Living Dungeon at `/living-dungeon`, and the configured wallet game at `/onchain`. The historical `/rise-testnet-demo` alias still resolves to the onchain route. The wallet implementation lives in `frontend/app/onchain-game.tsx`; importing the home, challenge, Practice or Living Dungeon route does not initialize the wallet bridge. See [The Living Dungeon](THE_LIVING_DUNGEON.md) for its Pact V0 rules and AI boundary, and [The Living Dungeon V2](THE_LIVING_DUNGEON_V2_PLAN.md) for the proposed conversation-first roadmap.
