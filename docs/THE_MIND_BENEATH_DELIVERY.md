...
## Gameplay audit — 21 September 2026

This pass repairs observed gameplay and consistency defects without changing other modes or adding a second language. It covers immutable event coordinates, actual character witnesses, forged evidence, surviving witness memories, sightlines for report interruption, objective-specific completion, truthful retreat, one-time memory rewards, recovery without resurrecting the room, traceable hazard damage, resource-bounded relic choices, player health rendering, reusable maneuvers, contextual buttons and suggestions, and superseded AI previews. Historical journals are preserved with the v4 replay boundary documented in the architecture file.

Initial verification: 385 unit tests passed, including 13 new focused audit regressions and the complete multi-Echo and legacy-save journeys. TypeScript passed. Lint has zero errors and the same 13 existing warnings outside Living Dungeon. Desktop visual play confirmed the rescue, correct next step and removal of the freed captive's Release action. Complete desktop/iPhone browser journeys and production verification are recorded below when complete.

The first complete iPhone run exposed an additional interaction defect: saving a maneuver triggered a smooth scroll while the player was clicking its correction. The trace showed no correction revision, and the independent Echo opening consequently never happened. Saving now leaves the editor in place; corrections display an explicit acknowledgment and appear in the maneuver's Why explanation. The browser test checks the correction revision before proceeding. The repeated complete iPhone expedition passed, including the independent opening and causal reconstruction. Desktop save/reload and guided progression were rechecked successfully. Both full desktop and iPhone paths now pass; the suite covers 20 browser cases, with focused repeats after the scroll fix.

The final candidate built successfully on Vercel. Browser inspection at 375 × 812 confirmed readable controls, an unobstructed board and no horizontal overflow. A real provider-generated rescue was executed successfully in the staged browser. The staging domain's deployment protection prevents unauthenticated API smoke checks; canonical endpoint verification follows promotion. Physical-device temperature and battery measurements are not available; reduced-motion and bounded-animation browser checks are covered.

Published to [delveworn.app/living-dungeon](https://delveworn.app/living-dungeon): deployment `dpl_CuEhLQ2iuNLopLA1H9FhXD32iXZ6`, [immutable build](https://delveworn-9idaw6zio-crypto-mickle.vercel.app/living-dungeon). Application changes correspond to `7513209` (audit engine commit `1302e7c`). The final Vercel production build, typecheck and targeted lint check passed. The three focused iPhone repeats and two desktop repeats all passed after the scroll fix.

Canonical production verification returned HTTP 200 for Living Dungeon, Practice, Challenge and Onchain. Three real AI calls returned English text, valid state bindings and executable certified plans; equivalent Norwegian and English input produced identical mechanical outcomes. This tests input interpretation, not an alternate locale: the game remains English only. Recorded usage: **3,121 input / 479 output tokens**, `gpt-5.6-terra`, latencies **3,014–4,084 ms**. Under the previously recorded assumption of $2/M input and $12/M output, these three requests would cost **$0.011990**; this is a conditional estimate, not a new price verification or billing total. The separately exercised staging-browser request and any development director calls are not included in this token subtotal. See [`THE_MIND_BENEATH_AUDIT_PRODUCTION_CHECK.json`](THE_MIND_BENEATH_AUDIT_PRODUCTION_CHECK.json).

The public browser resumed its actual pre-release save at revision 15 with the captive still freed, 40 health and 14 relic energy. A free examination advanced it to revision 16, saved successfully, and survived reload. The invalid Release action was absent. No other mode's implementation changed. Saving remains local to the browser, and the documented journal/AI limits and physical-device thermal measurement limitations still apply.


## Familiar monsters and Quartermaster Kevin — 21 September 2026

Published application commit `5c3a456`, deployment `dpl_ETNZZK78bnCGPPWFNw234LYCv8Ah`, [immutable build](https://delveworn-gythzasir-crypto-mickle.vercel.app/living-dungeon), promoted to [delveworn.app/living-dungeon](https://delveworn.app/living-dungeon). Test-selector correction: `d78a936`.

- Removed the Warden rendering path. Ordinary enemies reuse Gary, Brutus, Gribnob the Unqualified, Thud, Velvet Rot, Gronk and Miss Morgue from Delveworn's existing art. Board labels identify Goblin, Orc or Zombie; inspection, plans, observations, historical explanations and AI context use the matching named character. The Echo retains its own identity and existing adventurer silhouette.
- Quartermaster Kevin uses his original merchant cutout in the kiln captive role. Freeing him creates the existing kiln relationship and earns one favour. From chamber 5, his supply-route panel exposes potion delivery or report-conduit closure for one favour. The panel links back to the rescue through Why; resource caps, one repayment per chamber, spent debt, fallen players and running plans are reflected in available controls. Fulfilled favours update inventory or the physical report route and persist through reload.
- These are presentation identities over stable entity/relationship IDs. Existing journal facts, approved plan bindings and save checksums remain intact; v4 saves need no migration. Quoted player-authored maneuver names are preserved. The shared art implementations, other modes and English-only interface remain intact. The unused Warden pixels remain in the atlas with their original generation provenance; nothing renders that tile. No images were generated for this update.

Verification: all 386 unit/integration tests passed; eight relevant tests were rerun successfully after the historical witness-name presentation adjustment. TypeScript, lint (zero errors; 13 existing warnings elsewhere), local production build and Vercel production build passed. All 24 desktop/iPhone 11 Pro browser cases are verified, including both complete expeditions beyond the Echo, Kevin's rescue, both favour outcomes, reload, keyboard, touch, reduced motion and overflow checks. Three initial assertions matched both the visible repayment message and the same text inside collapsed Why history; the selector was anchored to the visible message, then both Kevin cases passed on both devices. No application change was needed for that test correction.

Desktop and mobile test screenshots were visually inspected for the monster labels and Kevin panel. The separate interactive Browser preview could not run because its administrative security-policy check was unavailable; no security control was disabled. This limitation does not replace the completed repository browser tests. Physical-device temperature and battery measurements remain unavailable.

Canonical production checks returned HTTP 200 for Living Dungeon, Practice, Challenge and Onchain. Two real provider requests explicitly named Gary and then Brutus / Quartermaster Kevin. Both returned English text, correct entity IDs and valid bindings; their plans completed the rescue without harming the monster when executed by the deterministic engine. Latencies: 3,307 and 3,767 ms. Explicit production-check usage: **2,076 input / 325 output tokens**, `gpt-5.6-terra`. Exact billed cost is not returned by the API; no new price assumption is presented. This subtotal covers these two explicit checks, not unrelated or development traffic. Evidence: [THE_MIND_BENEATH_CAST_PRODUCTION_CHECK.json](THE_MIND_BENEATH_CAST_PRODUCTION_CHECK.json).
