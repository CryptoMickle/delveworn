/** The original Market Dungeon boss score, shared with Delveworn. No fetched audio. */
...
export type BossScoreScheduler = {
...
export class BossBattleScore {
  ...
  private readonly scheduler: BossScoreScheduler;
  ...
  constructor(context: AudioContext, output: AudioNode, scheduler: BossScoreScheduler) {
  ...
  stop(immediate = false) {
    ...
    this.running = false;
    this.generation += 1;
    if (this.timer !== null) this.scheduler.clearTimer(this.timer);
    this.timer = null;
    const now = this.context.currentTime;
    this.bus.gain.cancelAndHoldAtTime(now);
  ...
  private scheduleNext(generation: number) {
    this.timer = this.scheduler.setTimer(() => {
      // A queued callback from an earlier encounter cannot start a second loop.
      if (generation !== this.generation || !this.running || this.destroyed) return;
      this.timer = null;
      try {
        this.schedule();
        this.scheduleNext(generation);
      } catch {
        try { this.stop(true); } catch { /* Output may have disappeared. */ }
        this.scheduler.onError?.();
      }
    }, SCHEDULER_MILLISECONDS);
  }
