...
import { transactionFailureMessage } from "./transaction-feedback";
...
function DelvewornGame() {
  ...
  const [actionFeedback, setActionFeedback] = useState("Choose an action. Results appear here after confirmation.");
  const {
    address:
      wagmiAddress,
    isConnected,
  ...
  */
  ...
  const audibleBoss = Boolean(player?.hasStarted && player.active && player.monsterType === 3 && player.monsterHp > 0 && connectedAddress);
  const { setBossBattle } = audio;
  useEffect(() => {
    setBossBattle(audibleBoss);
    return () => setBossBattle(false);
  }, [audibleBoss, setBossBattle]);
  ...
  */
  ...
  async function runRelicTransaction(
    ...
    if (interactionLock.current || !actionReady || canonicalSyncing || player.pendingRequestId > BigInt(0)) return;
    ...
    try {
      ...
      audio.playOutcome("relic");
      setActionFeedback(`Relic choice confirmed · ${getRelicDefinition(after.equippedRelic).name} equipped · HP ${after.hp}/${after.maxHp}.`);
      addMessages([
  ...
  */
  ...
  async function runSupplyTransaction(
    ...
    if (interactionLock.current || !actionReady || player.pendingRequestId > BigInt(0)) return;
    interactionLock.current = true;
    setWalletMessage("");
    audio.playAction("click");
  ...
  */
  ...
  async function runGameTransaction(
    ...
    try {
      ...
      // Only confirmed contract snapshots may produce result feedback.
      if (functionName === "attack" || functionName === "stormAttack" || functionName === "usePotion") {
        const result = functionName === "usePotion"
  ...
  */
  ...
  useLayoutEffect(() => {
    ...
    bossRewardRef.current?.scrollIntoView({ behavior: "instant", block: "start" });
  }, [
    player?.relicOfferAvailable,
    player?.relicOfferId,
    player?.supportsRelicCollection,
  ...
  */
  ...
  if (
    ...
    return (
      <main className="delveworn-onchain-mode min-h-screen bg-[#090909] text-white flex items-center justify-center">
        <div className="max-w-md px-6 text-center"><GameLogo /><p role="status" className="mt-6">Connecting to {ACTIVE_NETWORK_LABEL}…</p><p className="mt-3 text-sm text-zinc-400">Check your wallet for a request. You can return home while the connection loads.</p></div>
      </main>
    );
  }
          ...
          <section className="practice-main-card relative mb-4 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900">
            <DungeonEntry
              ...
              description={walletChoiceOpen
                ...
                : ACTIVE_ECOSYSTEM_NAME === "Somnia"
                  ? "Play with contract-backed state and verifiable randomness. Each move waits for confirmation. Practice is available from the menu for instant local play."
            ...
            >
              ...
              {walletMessage && <p role="status" className="mt-4 text-sm text-red-400">{walletMessage}</p>}
            </DungeonEntry>
          </section>
        </div>
      </main>
  ...
  if (
    ...
    return (
      <main className="delveworn-onchain-mode min-h-screen bg-[#090909] text-white flex items-center justify-center">
        <div className="max-w-md px-6 text-center"><GameLogo /><p role="status" className="mt-6">Reconnecting to {ACTIVE_NETWORK_LABEL}…</p><p className="mt-3 text-sm text-zinc-400">Your onchain player will appear after the next successful read.</p></div>
      </main>
    );
  }
  ...
  */
  ...
  const combatStatus = walletMessage || (busy
    ? actionProgressPhase === "preparing" || actionProgressPhase === "idle" && !randomnessPending
      ? "Check your wallet. Waiting for authorization; you may decline before sending."
      : actionProgressPhase === "sponsoring" ? "Preparing sponsored gas for your move…"
        : actionProgressPhase === "submitting" || actionProgressPhase === "inclusion" ? "Move sent. Waiting for inclusion onchain; keep this page open."
          : `${rollingTitle}. Waiting for the confirmed result; your last confirmed HP stays visible.`
    : actionFeedback);
  const combatActions = (
<CombatActionDock
            busy={busy}
            ...
            lastExchange={player.lastPlayerDamage || player.lastMonsterDamage || player.lastCritical ? { dealt: player.lastPlayerDamage, taken: player.lastMonsterDamage, critical: player.lastCritical } : undefined}
            retaliation={`${player.monsterDamageMin}–${player.monsterDamageMax}`}
            ...
            potionDisabledReason={player.potions === 0 ? "No potions left · restock with Kevin" : null}
  ...
  const recoveryEnterAction = (
    <button
      ...
      onClick={() => runGameTransaction("enterNextRoom")}
      disabled={busy}
    ...
    >
      {isBossRoom ? `ENTER BOSS ROOM ${currentRoom}` : `ENTER ROOM ${currentRoom}`}
    </button>
  );
  ...
  const recoveryHealAction = (
    ...
    >
      <div className="flex items-center justify-center gap-3">
        <span className="font-black">USE OWN POTION SAFELY · {player.potions}/{MAX_POTIONS}</span>
      </div>
      {(player.potions === 0 || player.hp >= player.maxHp) && <p className="mt-1 text-[10px] font-bold text-red-400">
        {player.potions === 0 ? "No potions available." : "HP is already full."}
      </p>}
    </button>
  );
  ...
  const recoveryLootDetail = player.lastLootType === 3
    ? `Weapon Level ${player.weaponLevel} · +${weaponBonus} DAMAGE`
    : player.lastLootType === 4
      ? `Armor Level ${player.armorLevel} · absorbs up to ${armorAbsorption} DAMAGE · max ${MAX_ARMOR_REDUCTION_PERCENT}% reduction`
      : player.lastLootType === 1
        ? `Potions: ${player.potions}/${MAX_POTIONS}`
        : player.lastLootType === 2 ? `Amount: +${player.lastLootAmount}` : undefined;
  const clearedMonster = getMonsterPersona(player.monsterType, Math.max(1, player.roomsCleared));

  const endedRunData: RunCardData = {
    mode: "onchain",
    networkLabel: ACTIVE_NETWORK_LABEL,
    roomsCleared: player.roomsCleared,
    gold: player.gold,
    weaponLevel: player.weaponLevel,
    armorLevel: player.armorLevel,
    bossesDefeated: Math.floor(player.roomsCleared / 10),
    relicName: equippedRelic.name,
    uniqueRelics: player.ownedRelics.length,
    totalRelicDrops,
  };

  const endedRestartAction = (
    <button
      type="button"
      data-keyboard-default="true"
  ...
  const endedCopyAction = (
    <RunResultShare
      label="COPY ONCHAIN RESULT"
      text={`Delveworn · ${ACTIVE_NETWORK_LABEL}\n${player.roomsCleared} rooms cleared · ${Math.floor(player.roomsCleared / 10)} bosses defeated · ${player.gold} gold remaining\nWeapon ${player.weaponLevel} · Armor ${player.armorLevel} · Relic: ${equippedRelic.name}\nThe dungeon has updated my performance review.\nStats copied from the last confirmed game snapshot; this text is not a proof certificate.\nContract: ${DUNGEON_EXPLORER_URL}\nPlay: ${siteUrl("/onchain")}`}
    />
  );
      ...
      <div className="practice-column mx-auto w-full max-w-md lg:max-w-6xl">
        ...
        {player.hasStarted && (
          <GameHud
            ...
            room={player.monsterHp === 0 ? Math.max(1, player.roomsCleared) : currentRoom}
            combatPotions={inCombat ? { used: player.combatPotionUses, limit: combatPotionLimit } : undefined}
          />
        )}

        {!inCombat && !recoveryActive && !endedActive && <div role="status" aria-live="polite" aria-atomic="true" className="onchain-action-feedback mb-3 min-h-[64px] rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 text-xs leading-relaxed text-zinc-300">
          {combatStatus}
        </div>}
        ...
        {player.hasStarted && (
          <RoomProgressLine
            ...
            roomsCleared={player.roomsCleared}
            isBoss={isBoss}
            phase={!player.active ? "Run ended" : player.relicOfferAvailable ? "Boss defeated · choose your relic" : player.campOpen ? "Kevin's camp · prepare for management" : player.supplyOpen ? "Kevin's supply stop" : roomCleared ? "Loot collected · recover before moving on" : isBoss ? "Boss encounter" : "Combat"}
          />
        )}
          ...
          {!player.hasStarted ? (
            ...
            <DungeonRunEnd
              data={endedRunData}
              restartAction={endedRestartAction}
              copyAction={endedCopyAction}
              log={combatLog}
              feedback={combatStatus}
            />

          ) : recoveryActive ? (
            <DungeonRecovery
              room={player.roomsCleared}
              lootType={player.lastLootType}
              lootAmount={player.lastLootAmount}
              lootDetail={recoveryLootDetail}
              fallbackArt={{ imageSrc: clearedMonster.image, name: clearedMonster.name }}
              flavor={player.campOpen
              ...
              potions={player.potions}
              maxPotions={MAX_POTIONS}
              merchant={player.campOpen || player.supplyOpen ? {
                name: MERCHANT_NAME,
                imageSrc: MERCHANT_IMAGE,
                kind: player.campOpen ? "camp" : "supply",
              } : undefined}
              enterAction={recoveryEnterAction}
              healAction={recoveryHealAction}
              shop={recoveryShop}
              relics={recoveryRelics}
              activeRelic={equippedRelic.name}
              ownedRelicCount={player.ownedRelics.length}
              relicsAvailable={player.supportsRelicCollection}
              log={combatLog}
              feedback={combatStatus}
            />
        ...
        {!recoveryActive && !endedActive && recoveryRelics}

        {/* ===================================================
            ACTIONS
        =================================================== */}

        {!player.hasStarted || endedActive ? (
          null
          ...
          */
          ...
          player.supportsRelicCollection && player.relicOfferId !== 0 ? (
            ...
            <div data-keyboard-action-scope>
              ...
              <div data-keyboard-actions data-keyboard-vertical="edges" className="grid gap-3">
        ...
        ) : !recoveryActive && (player.campOpen || player.supplyOpen) ? (
          <div data-keyboard-actions className="onchain-pending-recovery-actions">
            {recoveryShop}
            {recoveryHealAction}
            {recoveryEnterAction}
          </div>
        ...
        {player.hasStarted && !inCombat && !recoveryActive && !endedActive && (
          <DungeonLog
            entries={combatLog}
