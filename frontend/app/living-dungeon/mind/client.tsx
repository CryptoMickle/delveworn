  ...
  return <main className={`${styles.root} ${!run.relic.principles.length ? styles.onboarding : ""}`} lang="en" data-testid="mind-game" data-revision={run.revision} data-room={run.room.index} data-status={run.status} data-hp={run.player.hp}>
    ...
    <div className={styles.layout}>
      <section className={styles.worldColumn} aria-label="The room">
        ...
        {run.activePlan && <div className={styles.boardPlan}><div><span className={styles.eyebrow}>{run.activePlan.interrupted ? "INTERRUPTED" : "PLAN IN MOTION"}</span><b>{run.activePlan.interrupted ?? `Step ${run.activePlan.cursor + 1} of ${run.activePlan.plan.steps.length}`}</b></div><button onClick={() => { if (run.activePlan?.interrupted) { void dispatch({ type: "cancel" }); } else setAuto(v => !v); }}>{run.activePlan.interrupted ? "Improvise" : auto ? "Pause here" : "Continue here"}</button></div>}
      ...
      <aside className={styles.sideColumn}>
        ...
        {run.relic.principles.length > 0 && <>
          ...
          {tab === "memory" && <section className={styles.panel}><p className={styles.eyebrow}>THE STORY HAS MANY OWNERS</p><h3>Traces that matter</h3>{run.room.index >= 6 && run.hypotheses.map(h => <div className={styles.theory} key={h.claim}><span>THE DUNGEON SUSPECTS</span><p>{THEORY_COPY[h.claim]}</p><small>{h.confidence > .65 ? "Certain" : "Tentative"} · {h.rooms.length}  chambers · {h.noise > 2 ? "Conflicting traces" : "Consistent traces"}</small><Why run={run} sources={h.sources} /></div>)}
            ...
            {run.relationships.length > 0 && <><h3 className={styles.sectionTitle}>Those who remember you</h3>{run.relationships.map(r => <div key={r.id} className={styles.relationship}>◇ {r.name}<small>{r.rescued}  times freed · owes you {r.debt}  favours</small>{run.room.index >= 4 && r.debt > 0 && <div className={styles.contextActions}><button disabled={run.player.potions >= 4} onClick={() => void dispatch({ type: "favor", relationshipId: r.id, help: "supplies" })}>Ask for a potion · 1 favour</button><button disabled={!entity(run.room, "relay")?.active} onClick={() => void dispatch({ type: "favor", relationshipId: r.id, help: "silence" })}>Close the report route · 1 favour</button></div>}</div>)}</>}
            {run.echoes > 0 && <div className={styles.reconstruction}><h3>Why the Echo broke</h3>{([ ["The relic understood", run.chills.understood], ["The dungeon was wrong", run.chills.mistaken], ["The difference decided it", run.chills.divergence] ] as const).map(([label, id]) => <div key={label}><h4>{label}</h4><p>{id ? run.facts.find(f => f.id === id)?.text : "This path did not open during your expedition."}</p>{id && <Why run={run} sources={[id]} />}</div>)}</div>}
          </section>}
        </>}
...
function Why({ run, sources }: { run: Run; sources: string[] }) { return <details className={styles.why}><summary>Why?</summary>{traceSources(run, sources).slice(0, 10).map((line, i) => <p key={i}>{line}</p>)}</details>; }
