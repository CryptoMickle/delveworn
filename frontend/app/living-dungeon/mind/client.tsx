...
export default function MindBeneathClient() {
  ...
  const handleKey = useCallback((event: React.KeyboardEvent | KeyboardEvent) => {
    ...
    if (event.defaultPrevented || event.altKey || event.metaKey || event.ctrlKey || ["INPUT", "TEXTAREA", "SELECT"].includes(target.tagName) || target.isContentEditable) return;
  ...
  return <main className={`${styles.root} ${!run.relic.principles.length ? styles.onboarding : ""}`} lang="nb" data-testid="mind-game" data-revision={run.revision} data-room={run.room.index} data-status={run.status} data-hp={run.player.hp}>
    <header className={styles.header}>
      ...
      <div className={styles.headerActions}><button onClick={() => { void audio.current?.toggle().then(setSound); }} aria-label={sound ? "Slå av lyd" : "Slå på lyd"}>{sound ? "♪ Lyd på" : "♪ Lyd av"}</button><Link href="/">Alle moduser ↗</Link></div>
    </header>
    ...
    <div className={styles.layout}>
      <section className={styles.worldColumn} aria-label="Rommet">
        ...
        {plan && preview && <div className={styles.boardPlan}><div><span className={styles.eyebrow}>MULIG FRAMTID</span><b>{plan.steps.length} steg · {preview.energyCost} energi{preview.healthCost > 0 ? ` · ${preview.healthCost} liv` : ""}</b></div><button className={styles.primary} disabled={!preview.legal} onClick={() => { void dispatch({ type: "commit", plan }); setAuto(true); focusBoard(); }}>Forplikt deg →</button></div>}
          ...
          {tab === "room" && <section className={styles.panel}>
            {run.activePlan ? <div className={styles.activePlan}><p className={styles.eyebrow}>DIN HANDLINGSREKKE</p><h3>{run.activePlan.plan.name}</h3><p>{run.activePlan.interrupted ?? `Steg ${run.activePlan.cursor + 1} av ${run.activePlan.plan.steps.length}`}</p><div className={styles.buttonRow}><button className={styles.primary} disabled={!!run.activePlan.interrupted} onClick={() => { setAuto(false); void dispatch({ type: "step" }); }}>Neste steg →</button><button onClick={() => setAuto(v => !v)} disabled={!!run.activePlan.interrupted}>{auto ? "Pause" : "La planen gå"}</button></div><button className={styles.textButton} onClick={() => { setAuto(false); void dispatch({ type: "cancel" }); }}>Bryt av og improviser</button></div>
              ...
              : <><div className={styles.panelTitle}><span className={styles.eyebrow}>LES ROMMET</span><span>{pendingReports.length ? `${pendingReports.length} rapport på vei` : "Velg et publikum"}</span></div>
                ...
                <div className={styles.suggestions}><p className={styles.eyebrow}>TRE ÅPNINGER</p>{choices.map((choice, i) => <button key={choice.label} onClick={() => choosePlan(choice.operations, choice.boundary, choice.label)}><span>0{i + 1}</span><div><b>{choice.label}</b><small>{choice.detail}</small></div><span>↗</span></button>)}</div></>}
            ...
            {!run.activePlan && <div className={styles.intentInput}><label htmlFor="mind-intent">Hva prøver du å få til?</label><textarea id="mind-intent" rows={2} maxLength={AI_TEXT_LIMIT} value={text} onChange={e => setText(e.target.value)} placeholder="Få personen fri uten at skriveren ser hvordan." /><p>Teksten sendes til OpenAI når du ber om en mulig framtid. Den lagres ikke i statistikk.</p><button disabled={!text.trim() || aiBusy || callCount >= 40} onClick={() => void askRelic("plan")}>{aiBusy ? "Relikvien tenker. Du kan fortsatt gå." : "Vis en mulig framtid ↗"}</button>{aiLine && <p role="status">{aiLine}</p>}</div>}
          </section>}
