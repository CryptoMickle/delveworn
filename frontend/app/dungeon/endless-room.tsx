...
export function EndlessRoom({ mode, view, actions, enemyMaxHp, maxHp, gold, potions, roomTurns,
  ...
  const recoveryPotion = safeHealing && safePotion ? <InventoryPotions potions={potions} {...safePotion}
    disabledReason={view.pending ? "Finish the current action first." : safePotion.disabledReason ?? (view.hp >= maxHp ? "HP is already full." : null)} /> : null;
  ...
  const inventoryControl = recovery && relics
    ? <button type="button" className="dungeon-inventory-relics" onClick={() => open("relics")} aria-label="Open relic collection"><small>Relics</small><strong>View</strong></button>
    : <InventoryPotions potions={potions} />;
  ...
  const inventory = <div className="descent-mobile-inventory" role="group" aria-label="Inventory"><span><small>Gold</small><strong>{gold}</strong></span><span><small>Weapon</small><strong>+{view.weapon}</strong></span><span><small>Armor</small><strong>+{view.armor}</strong></span>{inventoryControl}</div>;
  const top = <div className="descent-mobile-top">
    ...
    {inventory}<DescentEnemyStatus name={view.enemyName} hp={view.enemyHp} maxHp={enemyMaxHp} incoming={incoming} isBoss={view.enemy === 3} />
  ...
  const footer = <div className="descent-mobile-footer">
    {!combat && <div className="descent-mobile-health"><div><span>HP <strong>{view.hp} / {maxHp}</strong></span>{health}</div></div>}
    {renderReport(true)}
    {recoveryPotion}
  </div>;
  ...
  return <section className="descent-shell endless-room" data-descent-phase={view.phase} data-game-mode={mode} data-room={view.room}>
    ...
    <div className="descent-hud"><div className="descent-vitality"><span>VITALITY <strong>{view.hp} / {maxHp}</strong></span>{health}</div><div><span>GOLD</span><strong>{gold}</strong></div><div><span>WEAPON</span><strong>{view.weapon}</strong></div><div><span>ARMOR</span><strong>{view.armor}</strong></div><div>{inventoryControl}</div></div>
